...
def fetch_smt_data(cfg, proxies):
    ...
    elif cfg.has_option('server', 'metadata_file'):
        local_filename = cfg.get('server', 'metadata_file')
        if os.path.exists(local_filename):
            read_config = configparser.ConfigParser()
            read_config.read(local_filename)
            expected_entries = ('SMTserverIP', 'SMTserverName', 'fingerprint')
            smt_info_xml = '<regionSMTdata><smtInfo '
            for attr in expected_entries:
                value = read_config.get("exoscale-smt", attr)
                if not value:
                    logging.error(
                        'local config file did not supply a value for "%s"' % attr
                    )
                    logging.error('Cannot proceed, exiting registration')
                    sys.exit(1)
                smt_info_xml += '%s="%s" ' % (attr, value)
